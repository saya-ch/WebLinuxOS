...
registerCommand('color', {
  handler: async (context: CommandContext): Promise<CommandResult> => {
    const { args } = context
    const color = args[0] || Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0')
    
    const hex = color.startsWith('#') ? color.slice(1) : color
    
    if (!/^[0-9a-fA-F]{6}$/.test(hex)) {
      return { output: 'color: 无效的十六进制颜色\n用法: color [RRGGBB]' }
    }
  ...
  description: '颜色值转换器',
  usage: 'color [RRGGBB]',
  examples: ['color FF5733', 'color 3498db', 'color']
})
...
registerCommand('uuid', {
  handler: (): CommandResult => {
    ...
    const output = [
      ...
      `版本:    4 (随机)`,
      `UUID:    ${uuid}`,
      `大端格式: 0x${uuid.replace(/-/g, '').toUpperCase()}`,
      '',
  ...
  handler: (context: CommandContext): CommandResult => {
    ...
    if (mode === 'decode') {
      try {
        ...
        return { output: 'base64: 解码失败，请检查输入是否为有效的Base64字符串' }
      }
    }
    
    return { output: 'base64: 未知操作\n用法: base64 [encode|decode] <文本>' }
  },
...
registerCommand('hash', {
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    if (!validAlgorithms.includes(algo)) {
      return { output: `hash: 不支持的算法 "${algo}"\n支持: ${validAlgorithms.join(', ')}` }
    }
    ...
    try {
      ...
      const output = [
        ...
        `原文:   ${text}`,
        `哈希值: ${hashHex}`,
        `长度:   ${hashHex.length} 字符`,
      ]
      
      return { output: output.join('\n') }
    } catch {
      return { output: 'hash: 计算失败' }
    }
  },
  description: '文本哈希计算',
  usage: 'hash <算法> <文本>',
  examples: ['hash SHA-256 Hello', 'hash SHA-512 password']
})
...
registerCommand('qr', {
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      return { output: 'qr: 生成失败' }
    }
  },
  description: '生成二维码',
  usage: 'qr <文本或URL>',
  examples: ['qr https://github.com', 'qr Hello']
})
