...
registerCommand('nasa', {
  handler: async (): Promise<CommandResult> => {
    try {
      const response = await fetchWithTimeout(`${API_CONFIG.nasa.baseUrl}/apod?api_key=${API_CONFIG.nasa.key}`)
...
registerCommand('weather-forecast', {
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    const cityInfo: Record<string, { lat: number; lon: number; name: string }> = {
    ...
    const info = cityInfo[city.toLowerCase()] || { lat: 39.9042, lon: 116.4074, name: city }
    
    try {
      const response = await fetchWithTimeout(
        `${API_CONFIG.openMeteo.baseUrl}/forecast?latitude=${info.lat}&longitude=${info.lon}&daily=temperature_2m_max,temperature_2m_min,weather_code&timezone=Asia/Shanghai&forecast_days=7`
      )
      ...
      const weatherDescriptions: Record<number, string> = {
        0: '☀️ 晴朗', 1: '⛅ 多云', 2: '☁️ 阴天', 3: '🌧️ 小雨',
        45: '🌫️ 雾', 48: '🌫️ 霜雾', 51: '🌧️ 小雨', 53: '🌧️ 中雨',
        55: '🌧️ 大雨', 61: '🌧️ 阵雨', 63: '🌧️ 强阵雨', 65: '🌧️ 猛烈阵雨',
        71: '❄️ 小雪', 73: '❄️ 中雪', 75: '❄️ 大雪', 80: '🌦️ 阵雨',
        81: '🌦️ 强阵雨', 82: '🌧️ 暴雨', 95: '⛈️ 雷暴', 96: '⛈️ 雷暴伴冰雹',
        99: '⛈️ 强雷暴伴冰雹',
      }
      ...
      const output = [
        ...
        `城市: ${info.name}`,
        '',
        `${'日期'.padEnd(12)} ${'天气'.padEnd(10)} ${'最高'.padEnd(8)} ${'最低'}`,
        ...((data.daily?.time || []).map((date: string, i: number) => {
          ...
          const max = data.daily?.temperature_2m_max?.[i] || 'N/A'
          const min = data.daily?.temperature_2m_min?.[i] || 'N/A'
          return `${new Date(date).toLocaleDateString('zh-CN').padEnd(12)} ${desc.padEnd(10)} ${max.toString().padEnd(6)}°C ${min}°C`
        }) || []),
        '',
        '数据来源: Open-Meteo',
      ]
...
registerCommand('starwars', {
  handler: async (context: CommandContext): Promise<CommandResult> => {
    const { args } = context
    const type = args[0]?.toLowerCase() || 'people'
    const id = args[1] || Math.floor(Math.random() * 10 + 1).toString()
    ...
    try {
      const response = await fetchWithTimeout(`${API_CONFIG.starWars}/${endpoint}/${id}`)
      ...
      if (type === 'people') {
        return {
          output: [
            ...
            `发色: ${data.hair_color}`,
            `眼睛颜色: ${data.eye_color}`,
            `出生年份: ${data.birth_year}`,
            `性别: ${data.gender}`,
            `所属星球: ${data.homeworld ? data.homeworld.split('/').pop() : '未知'}`,
            '',
            '数据来源: SWAPI',
          ].join('\n')
        }
  ...
  description: '查询星球大战数据',
  usage: 'starwars <类型> [ID]',
  examples: ['starwars', 'starwars people 1', 'starwars planets']
})
