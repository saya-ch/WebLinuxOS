...
- **642+ 内置应用** — 涵盖开发、生产力、AI、互联网、数据分析、工具和游戏
...
- **AmbientSound 环境音播放器** — Web Audio API 实时合成 12 种环境音效（雨声、雷声、风声、火焰、海浪、森林、鸟鸣、咖啡馆、白噪声、粉噪声、键盘、钟表），支持多音混合、独立音量控制、5 种预设场景、定时关闭
- **MarkdownNotebook 智能笔记本** — 多文档管理 + 文件夹分类 + 分屏 Markdown 编辑/预览 + 收藏 + 实时字数统计 + HTML 导出 + 深色/浅色主题
- **CSP 安全配置优化** — Content-Security-Policy 从 JavaScript 动态注入改为 HTML head 静态声明，确保在页面加载前即生效
- **Service Worker 缓存版本更新** — 缓存策略升级，确保新版本资源及时更新

### v129 新增亮点

- **文本编辑器语法高亮** — 基于 overlay 技术的多语言语法高亮（JS/TS/Python/Go/Rust/HTML/CSS/Shell/SQL），支持 60+ 文件类型自动检测
- **DeployMonitor 部署监控面板** — 实时查看 GitHub Pages 部署状态、Workflow 运行历史、成功率统计
- **SmartTranslator 智能翻译器** — 基于 MyMemory 免费 API 的 20 种语言实时互译，支持语音朗读和翻译历史
- **终端新命令** — `json`（JSON 格式化/验证）、`stats`（文本统计）、`uuid`（UUID 生成）、`checksum`（哈希校验）、`colors`（颜色调色板）、`repeat`（重复输出）
- **键盘快捷键修复** — TextEditor 全局快捷键仅在获得焦点时触发，避免与其他应用冲突
...
- MarkdownNotebook 智能笔记本（分屏编辑/预览、导出）
...
- AmbientSound 环境音播放器（Web Audio 实时合成）
- 音乐工作室、音频可视化、画板
- 屏幕录制、相机、视频播放器
...
200+ 内置命令，涵盖文件操作、系统信息、网络诊断、AI 对话、API 调用、加密、JSON 处理、文本统计、哈希校验等。基于虚拟文件系统构建，支持命令历史、Tab 补全和管道操作。
