...
WebLinuxOS 支持大量键盘快捷键。全局快捷键在桌面环境生效，窗口快捷键在窗口获得焦点时生效。
...
| 快捷键 | 功能 |
|--------|------|
| `Alt + Tab` / `Alt + Shift + Tab` | 切换窗口（反向） |
| `Ctrl/⌘ + W` | 关闭当前窗口 |
| `Ctrl/⌘ + M` | 最小化当前窗口 |
| `Ctrl/⌘ + Shift + M` | 最大化当前窗口 |
| `Ctrl/⌘ + Q` | 关闭当前窗口 |
| `Esc` | 还原最大化窗口 / 退出输入焦点 |
| `F11` | 最大化当前窗口 |
| `PrintScreen` | 打开截图应用 |
...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl + Alt + 1..9` | 切换到指定虚拟桌面 |
| `Ctrl + Alt + ←/→` | 切换到上一个/下一个虚拟桌面 |
| `Ctrl + Shift + Alt + 1..9` | 将当前窗口移动到指定虚拟桌面 |
| `Ctrl + Shift + Alt + ←/→` | 将当前窗口移动到上一个/下一个虚拟桌面并跟随 |
| `Ctrl + Shift + ↑/↓` | 在同一应用的窗口间切换 |

### 提示

- 在 `INPUT / TEXTAREA / contenteditable` 元素中输入文本时，大多数全局快捷键会被暂停，避免干扰正常输入；此时仅 `Esc` 可用于失焦。
- 窗口拖拽与缩放操作会被节流到合理的帧率（约 30 FPS），减少不必要的主线程计算。
...
**版本**: 6.3.1 &nbsp;|&nbsp; **最后更新**: 2026-06-18 &nbsp;|&nbsp; **作者**: saya-ch
