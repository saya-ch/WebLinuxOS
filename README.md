...
WebLinuxOS 是一个完整的 Linux 桌面环境模拟器，完全运行在浏览器中，无需后端依赖。它具有完整的窗口管理系统、虚拟文件系统、带40+内置命令的终端模拟器，以及**59个预装应用程序**（新增剪贴板管理器、快捷命令和智慧搜索），涵盖系统工具、办公工具、互联网客户端、多媒体编辑器、开发工具和游戏。
...
- 新增剪贴板管理器、快捷命令和智慧搜索到桌面图标
- 全局搜索和智慧搜索界面采用现代化的玻璃态设计
- 剪贴板管理器支持实时监听，自动记录剪贴板内容
- 智慧搜索提供统一的搜索入口，提升用户效率
...
- 优化了 Vite 构建配置，修复了 manualChunks 类型问题
- 改进了 NotificationSystem 组件导入路径
...
| **Smart Search** | 全局智慧搜索，支持跨应用、文件、命令和设置搜索 ✨新功能 |
